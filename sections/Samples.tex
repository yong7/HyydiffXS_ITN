...
Events are selected if they pass either a diphoton trigger or single photon trigger\cite{egamma_trigger}.The diphoton trigger has a transverse energy thresholds of 35GeV and 25GeV for the leading and sub-leading photon candidates respectively,with medium photon identification based on calorimeter shower shape variables.The single-photon trigger require the transverse energy of the leading photon greater than 140 GeV and pass the loose photon identification.On average,the trigger efficiency is greater than 98\% for the events pass the di-photon event selection.However,The additional improvement from the single photon trigger is less than 1\% . Table ~\ref{tab:trig} summarized the HLT diphoton trigger for each year. 
...
The standard model predicted cross-sections at 13.6 TeV have been calculated in twiki \cite{higg_xs}.The predict cross-section times branching ratios of $H\to \gamma\gamma$ and total events produced in 168~\ifb are summarized in Table \ref{tab:samples:signal_xsecbr}.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lrll}
    ...
    %\hline
    \toprule
    Production mode & $\sigma\times BR$ [fb] & accuracy (of $\sigma$) & total events produced in 168~\ifb \\
    %\hline
    \midrule
