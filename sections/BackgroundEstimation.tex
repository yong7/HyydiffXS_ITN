...
The fractions contributed by the \(\gamma\gamma\), \(\gamma j\) and \(jj\) background sources after the inclusive diphoton selection are \ASnote[\SI{0.0}{\percent}, \SI{0.0}{\percent} and \SI{0.0}{\percent}, respectively, as shown in Figures XXX]{Compute \& plot inclusive background fractions for full 2022-24 dataset}. 

Systematic uncertainty on the background fractions is estimated by varying the loosened identification pre-selection from \texttt{LoosePrime3} to \texttt{LoosePrime4} and \texttt{LoosePrime5}. These alternative working points are defined as follows: \ASnote[\texttt{LoosePrime4}]{Add LP4 definition here}, \ASnote[\texttt{LoosePrime5}]{Add LP5 definition here}. ASnote{Include table with systematics}
