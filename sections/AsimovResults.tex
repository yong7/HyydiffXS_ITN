...
to verify this is a numerical coincidence rather than a bug. Specifically, as expected the ttH region contributions are predominantly from the ttH production, 
...
The composition of the total uncertainty for each of the cross-sections can be studies by performing the signal extraction fits with 
...
and  MET, Lepton and ttH  selections in Figure~\ref{fig:fraction_sys_errorgroup_bin_by_bin_AsimovSB_2}. 
...
\ref{fig:fraction_sys_errorgroup_bin_by_bin_bin_by_bin_AsimovSB_diff_3} show the uncertainty composition for the differential observables. For these, the spurious 
...
\ygg{} with roughly diagonal correlation matrix and Njets with non-diagonal correlation matrix. 
