...
The $\gamma\gamma$-only MC template is currently used for the spurious signal test (\textcolor{red}{to be updated to the full template following background decomposition}). Figure~\ref{fig:ss_Diphoton_fid_main} presents the spurious signal uncertainties in the inclusive diphoton region for various test background functions.The red dashed line indicates the $\pm$10\% threshold range for the second criterion, which is the case for most of the background functions selected.
...
\begin{figure}[htb!]
  ...
  \label{fig:ss_Diphoton_fid_main}
\end{figure}
