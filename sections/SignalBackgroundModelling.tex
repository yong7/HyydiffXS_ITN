...
This $p$-value is evaluated on the un-smoothed template.For the functions that pass the spurious signal test on the smoothed template, if the $p$-value of the first-choice function on the un-smoothed template is lower than 1\%, the next function in the priority list is tested until a function with a $p$-value greater than 1\% is found.. For the function changed due to the $p$-value. the corresponding bins are labelled with "p($\chi^{2}$) change". 
 If all pass functions with a lower $p$-value, dedicated toy studies are performed to verify that the selected background function is a good choice. The corresponding bins are labeled with "toy studies" and the detail explanation in \cite{HIGG-2019-13-INT1}.currently no corresponding bins for this case.
