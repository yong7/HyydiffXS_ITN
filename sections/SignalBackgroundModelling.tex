...
This $p$-value is evaluated on the un-smoothed template.For the functions that pass the spurious signal test on the smoothed template, if the $p$-value of the first-choice function on the un-smoothed template is lower than 1\%, the next function in the priority list is tested until a function with a $p$-value greater than 1\% is found. For the function changed due to the $p$-value. The corresponding bins are labeled with "p($\chi^{2}$) change". 
