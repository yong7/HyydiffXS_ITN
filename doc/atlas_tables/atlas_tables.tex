...
\Option{table-number-alignment} is particulalrly useful if the column header is wider then the conent.
...
The macro \Macro{numpmerrx} uses \Macro{supsub} and has two optional arguments:
...
\begin{tcblisting}{}
  \caption{Numbers with asymmetric uncertainties.
    The last two rows in the \Macro{numpmerr} column
    use the \Macro{numpmRP} and \Macro{numpmRF} macros, respectively.}%
  \label{tab:asym}
  \centering
  \renewcommand{\arraystretch}{1.4}
  \begin{tabular}{cccc}
